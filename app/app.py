...
def mol_changed():
    ...
    dih = {}
    mol = Chem.MolFromSmiles(inString) if smiles_to_amsr else amsr.ToMol(inString, dihedral=dih)
    if mol_isOK(mol):
        ...
        if threeD:
            mol, ener = amsr.GetConformerAndProperties(mol, dihedral=dih)
    ...
    return json.dumps({"svg": svg, "sdf": sdf, "outString": outString, "ener": ener, "qed": QED, "tpsa": tpsa, "sa": sa})
