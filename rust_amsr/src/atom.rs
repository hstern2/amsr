...
impl Atom {
    pub fn new(symbol: &str) -> AMSRResult<Self> {
        let mut atom = Atom {
    ...
    fn parse_symbol(&mut self) -> AMSRResult<()> {
    ...
    fn calculate_properties(&mut self) -> AMSRResult<()> {
        ...
        let valence = get_valence(&self.atom_symbol, self.charge, self.bangs)
            .ok_or_else(|| AMSRError::ValenceError(format!("Unknown valence for {}{}{}", 
        ...
        if self.max_neighbors < 0 {
            return Err(AMSRError::ValenceError(format!("Invalid valence state for atom {}", self.symbol)));
        }
