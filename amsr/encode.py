...
def FromMolToTokens(mol):
    ...
    def searchOrder(b,a):
        ...
        c = b.GetOtherAtom(a)
        isSeen = seen(c)
        return (not isSeen, visitedIndex(a, atom) - visitedIndex(c, atom) if isSeen else c.GetIdx())
    def search(a):
        nonlocal nSeenAtoms
        i = a.GetIdx()
        si = atom[i]
        ...
        for b in sorted(a.GetBonds(), key=lambda b: searchOrder(b,a)):
            ...
            if seen(c): # ring
                nSkip = 0
                for n in sorted(BFSFind(a,j,seenBonds), key=byVisitedIndex, reverse=True):
                    k = n.name.GetIdx()
                    if k == j:
                        yield (b, bond)
