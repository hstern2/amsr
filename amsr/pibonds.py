...
def piBond(mol,atom,i,j,order):
    mol.GetBondBetweenAtoms(i,j).SetBondType(Chem.BondType.TRIPLE if order == 3 else Chem.BondType.DOUBLE)
...
def PiBonds(mol, atom):
    def canPiBond(i): return atom[i].nAvailablePiBonds() >= 1
    def canMultiplePiBond(i): return atom[i].nAvailablePiBonds() >= 2
    def canBePi(i,j): return canPiBond(i) and canPiBond(j)
    def canBeTriple(i,j): return canMultiplePiBond(i) and canMultiplePiBond(j)
    def singleCoordinate(g,i): return g.degree[i] == 1
    def possiblePiBonds():
